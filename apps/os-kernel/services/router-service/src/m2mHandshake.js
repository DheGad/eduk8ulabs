...
import { getAgent, touchAgent } from "./agentRegistry.js";
// ─── State ────────────────────────────────────────────────────────────────────
 ...
 */
export async function brokerHandshake(agentA_id, agentB_id, payload_hash) {
    ...
    if (!agentA)
        return deny(`Unknown agent: ${agentA_id}`, false);
    if (!agentB)
        return deny(`Unknown agent: ${agentB_id}`, false);
    if (!agentA.active)
        return deny(`Agent ${agentA_id} is deactivated`, true);
    if (!agentB.active)
        return deny(`Agent ${agentB_id} is deactivated`, true);
    ...
    if (agentA.owner_tenant !== agentB.owner_tenant) {
        ...
        if (!aCan || !bCan) {
            return deny(`Cross-tenant M2M blocked. ${!aCan ? agentA_id : agentB_id} lacks CROSS_TENANT_BRIDGE scope.`, true);
        }
    }
    // 3. Clearance level compatibility (A must not exceed B's clearance, and vice versa)
    const CLEARANCE_ORDER = { PUBLIC: 0, INTERNAL: 1, CONFIDENTIAL: 2, TOP_SECRET: 3 };
    ...
    if (Math.abs(aLevel - bLevel) > 1) {
        return deny(`Clearance mismatch: ${agentA.clearance_level} ↔ ${agentB.clearance_level} — gap exceeds one level.`, true);
    }
    // 4. Scope overlap — agents must share at least one compatible scope
    const sharedScopes = agentA.allowed_scopes.filter(s => agentB.allowed_scopes.includes(s));
    if (sharedScopes.length === 0) {
        return deny(`No overlapping scopes between ${agentA_id} [${agentA.allowed_scopes.join(",")}] and ${agentB_id} [${agentB.allowed_scopes.join(",")}].`, false);
    }
    ...
    console.info(`[V41:M2MHandshake] ✅ APPROVED ${handshake_id}: ${agentA.name} ↔ ${agentB.name} | ` +
 ...
 */
export function consumeContractToken(token) {
    const entry = tokenLedger.get(token);
    if (!entry)
        return null;
    if (entry.used) {
        tokenLedger.delete(token);
        return null;
    }
...
// ─── Monitoring ───────────────────────────────────────────────────────────────
function logHandshakeEvent(handshake_id, a, b, status, brokered_at, reason) {
    handshakeLog.unshift({ handshake_id, agent_a: a, agent_b: b, status, brokered_at, reason });
    if (handshakeLog.length > 100)
        handshakeLog.pop(); // Keep last 100
}

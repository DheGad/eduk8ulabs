...
export interface TelemetryPayload {
  ...
  compliance_rating:      string;   // "A+", "A", "B", etc.
  // Time-series (7-day)
...
const TENANT_PROFILES: Record<string, {
  volumeBase:     number;
  threatRate:     number;   // 0-1, proportion of requests that are threats
  latencyBase:    number;
  trustBase:      number;   // 80–99
...
const SECURITY_EVENT_DESCRIPTIONS: Record<SecurityEvent["type"], string[]> = {
  V12_POLICY_DENY: [
...
export function generateTelemetry(tenantId: string, periodDays = 7): TelemetryPayload {
  ...
  // ── Time-Series ───────────────────────────────────────────────────────────
  const now        = new Date();
  const timeSeries: DailyDataPoint[] = [];
  ...
  const severities: SecurityEvent["severity"][] = ["CRITICAL", "HIGH", "HIGH", "MEDIUM", "LOW"];

  const recentEvents: SecurityEvent[] = Array.from({ length: 8 }, (_, i) => {

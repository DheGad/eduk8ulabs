...
export function updatePlugin(id, patch) {
    ...
    if (!existing)
        throw new Error(`[V39:PluginRegistry] Plugin "${id}" not found.`);
 ...
 */
export async function runPlugins(phase, ctx) {
    let currentCtx = { ...ctx };
    for (const plugin of plugins.values()) {
        ...
        if (plugin.phase !== phase)
            continue;
        if (plugin.tenant_scope !== "*" && plugin.tenant_scope !== ctx.tenant_id)
            continue;
        try {
            currentCtx = await plugin.handler(currentCtx);
        }
        catch (err) {
            console.error(`[V39:PluginRegistry] Plugin "${plugin.id}" threw during ${phase}: ${err.message}. ` +
...
registerPlugin({
    ...
    handler: async (ctx) => {
        ...
        if (promptLevel < 0)
            return ctx; // Unknown classification — pass through

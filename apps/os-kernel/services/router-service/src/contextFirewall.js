 ...
 */
export function abstractContext(text) {
    const contextMap = {};
    let roleCounter = 0;
    let orgCounter = 0;
    let locCounter = 0;
    ...
    const roleOriginalToPlaceholder = {};
    ...
    const orgOriginalToPlaceholder = {};
    ...
    // ── Pass 3: Locations ─────────────────────────────────────────────────────
    const locOriginalToPlaceholder = {};
 ...
 */
export function restoreContext(text, contextMap) {
    if (!text || Object.keys(contextMap).length === 0)
        return text;
    let restored = text;
...
if (isMain) {
    ...
    for (const tc of testCases) {
        ...
        const hasPlaceholder = abstractedText.includes("[ROLE_") ||
        ...
        if (hasPlaceholder && roundTrip) {
            ...
            failed++;
            if (!hasPlaceholder)
                console.log("   ⚠ WARNING: No placeholders were inserted.");
            if (!roundTrip)
                console.log("   ⚠ WARNING: Round-trip failed.");
        }
    }

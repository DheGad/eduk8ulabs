...
export interface FragmentManifest {
  /** Non-guessable manifest ID — used to reassemble after execution */
  manifest_id:  string;
  /** Total number of shards created */
  shard_count:  number;
  /** Whether sensitive entities were detected and fragmented */
  fragmented:   boolean;
  /** List of entity types that were detected (e.g. "ACCOUNT_NUMBER", "SSN") */
  entity_types: string[];
  /** Individual shard descriptors — the shards themselves are kept in ephemeralMemory */
...
export interface ShardDescriptor {
  ...
  order_index: number;
  size_chars:  number;
  /** HMAC hash of this shard — proves integrity without revealing content */
...
const fragmentStore = new Map<string, { shards: string[]; manifest: FragmentManifest }>();
...
export function fragmentPrompt(
  prompt: string,
): { manifest: FragmentManifest; reassembledForExecution: string } {
  const detectedTypes: string[] = [];

  for (const { name, pattern } of SENSITIVE_PATTERNS) {
    if (pattern.test(prompt)) detectedTypes.push(name);
  }
  ...
  const shardDescriptors: ShardDescriptor[] = rawShards.map((shard, i) => ({
  ...
  console.info(
    `[V37:Fragmentation] Manifest ${manifestId} — ` +
...
export function getFragmentStoreSize(): number {
  return fragmentStore.size;
}

/** Called by ephemeralMemory.zeroize() — removes all shard data for a manifest */
export function destroyManifest(manifest_id: string): boolean {

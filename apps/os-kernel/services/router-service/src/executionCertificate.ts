...
export interface ExecutionCertificate {
  /** Globally unique, non-guessable identifier for this execution */
  execution_id: string;
  /** ISO-8601 timestamp of certificate issuance */
  issued_at: string;
  /** V25 Global Trust Score (0–100) */
  trust_score: number;
  /** Human-readable trust band colour */
  trust_band: "PLATINUM" | "GOLD" | "SILVER" | "BRONZE" | "CRITICAL";
  /** Any compliance flags raised during this execution */
  compliance_flags: string[];
  /** AWS region / enclave zone the execution ran in */
  region: string;
  /** Model identifier used for this execution */
  model: string;
  /** Provider of the model */
  provider: string;
  /** HMAC-SHA256 of the canonical payload — proves certificate integrity */
  zk_signature: string;
  /** Short public fingerprint (first 12 hex chars of signature) for display */
  fingerprint: string;
}
...
const certLedger = new Map<string, ExecutionCertificate>();
 ...
 */
function deriveTrustBand(score: number): ExecutionCertificate["trust_band"] {
...
export function issueCertificate(params: {
  ...
  const cert: ExecutionCertificate = {
    execution_id,
    issued_at,
...
export function verifyCertificate(
...
): { status: "SECURE" | "TAMPERED"; cert: ExecutionCertificate } | null {
  ...
  let status: "SECURE" | "TAMPERED" = (expectedSig === cert.zk_signature)
    ? "SECURE"
    : "TAMPERED";
...
export function getCertFromLedger(execution_id: string): ExecutionCertificate | null {

...
function encryptShare(plainBase64: string): { encrypted: string; iv: string; auth_tag: string } {
...
function decryptShare(encrypted: string, ivHex: string, authTagHex: string): string {
 ...
 */
sovereigntyRouter.post(
  "/api/v1/sovereignty/revoke",
  async (req: Request, res: Response): Promise<void> => {
    const { user_id, initiated_by = "user" } = req.body as {
      user_id?: string;
      initiated_by?: string;
    };
    ...
    try {
      ...
      const countResult = await pool.query<{ count: string }>(
        `SELECT COUNT(*) AS count FROM shamir_shard_custody WHERE user_id = $1`,
        [uid],
      );
      ...
      await pool.query(
        `DELETE FROM shamir_shard_custody WHERE user_id = $1`,
        [uid],
      );

      console.warn(`[Sovereignty] User ${uid}: ${shardsDeleted} shard rows purged from DB`);
    } catch (dbErr) {
      console.error("[Sovereignty] DB shard purge failed:", (dbErr as Error).message);
      res.status(500).json({
        success: false,
    ...
    try {
      ...
      console.error("[Sovereignty] revocation_log write failed (non-fatal):", (logErr as Error).message);
    }
 ...
 */
sovereigntyRouter.post(
  "/api/v1/sovereignty/kms/link",
  async (req: Request, res: Response): Promise<void> => {
    const { user_id, provider, key_arn, display_name, role_arn } = req.body as {
      user_id?:     string;
      provider?:    string;
      key_arn?:     string;
      display_name?: string;
    ...
    try {
      ...
      console.error("[Sovereignty] KMS link failed:", (dbErr as Error).message);
      res.status(500).json({
        success: false,
...
sovereigntyRouter.get(
  "/api/v1/sovereignty/kms/status",
  async (req: Request, res: Response): Promise<void> => {
    ...
    try {
      const result = await pool.query(
        `SELECT id, provider, key_arn, display_name, is_attested, attested_at,
...
sovereigntyRouter.get(
  "/api/v1/sovereignty/shards/count",
  async (req: Request, res: Response): Promise<void> => {
    const { user_id } = req.query as { user_id?: string };
    if (!user_id) {
      res.status(400).json({ success: false, error: { code: "INVALID_PAYLOAD", message: "user_id required" } });
 ...
 */
sovereigntyRouter.post(
  "/api/v1/sovereignty/shards/store",
  async (req: Request, res: Response): Promise<void> => {
    if (!requireInternalAuth(req, res)) return;

    const { user_id, shares, kms_key_id } = req.body as {
      user_id:    string;
      shares:     Record<string, { share2: string; share3: string }>;
      kms_key_id?: string;
    };
    ...
    const rows: Array<[string, string, string, string, string, string, string, string, string | null]> = entries.map(
      ([token, { share2, share3 }]) => {
        ...
        return [
          ...
          kms_key_id ?? null,
        ] as [string, string, string, string, string, string, string, string, string | null];
      }
    );
    ...
    try {
      ...
      for (const [uid, token, s2enc, s2iv, s2tag, s3enc, s3iv, s3tag, kmsId] of rows) {
        await pool.query(
          `INSERT INTO shamir_shard_custody
           ...
           ON CONFLICT (user_id, token) DO UPDATE SET
             ...
             updated_at        = NOW()`,
          [uid, token, s2enc, s2iv, s2tag, s3enc, s3iv, s3tag, kmsId],
        );
      }

      res.status(200).json({ success: true, stored: entries.length });
    } catch (err) {
      console.error("[Sovereignty] Shard store failed:", (err as Error).message);
 ...
 */
sovereigntyRouter.post(
  "/api/v1/sovereignty/shards/retrieve",
  async (req: Request, res: Response): Promise<void> => {
    if (!requireInternalAuth(req, res)) return;

    const { user_id, tokens } = req.body as { user_id: string; tokens: string[] };
    ...
    try {
      const result = await pool.query<{
        token: string;
        share_2_encrypted: string;
        share_2_iv: string;
        share_2_auth_tag: string;
      }>(
        `SELECT token, share_2_encrypted, share_2_iv, share_2_auth_tag
      ...
      for (const row of result.rows) {
        try {
          externalShares[row.token] = decryptShare(
            row.share_2_encrypted,
            row.share_2_iv,
            row.share_2_auth_tag,
          );
        } catch {

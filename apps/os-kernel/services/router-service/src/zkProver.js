...
import { computeLeafHash } from "./merkleLogger.js";
 ...
 */
function deriveG1Point(seed, label) {
 ...
 */
function deriveG2Point(seed, label) {
 ...
 */
function computeVkeyBinding(proof, signals) {
 ...
 */
function computePublicSignals(receipt, ctx) {
    return [
 ...
 */
function _simulateGroth16Proof(receipt, ctx) {
    ...
    const witnessSeed = sha256hex(`${receipt.signature}|${ctx.merkle_leaf_hash}|${ctx.tenant_id}|${CIRCUIT_VERSION}`);
    return {
 ...
 */
export function generateZkProof(receipt, ctx) {
 ...
 */
export function verifyZkProof(credential) {
...
function _verify(proof, signals, storedBinding) {

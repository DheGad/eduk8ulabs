...
export type ContainmentStatus = "GREEN" | "YELLOW" | "RED";
export type TripwireReason =
  | "RAPID_ENUMERATION"          // >50 requests/sec from one tenant
  | "ANOMALOUS_RESPONSE_HARVEST" // Large response extraction pattern
  | "POLICY_BYPASS_ATTEMPT"      // V12 policy overridden from response
  | "EXECUTION_STORM"            // Unusual concurrent execution spike
  | "FOREIGN_ROUTING_INJECTION"; // Model override attempt
...
export interface ContainmentEvent {
  ...
  triggered_at:    string;
  status:          "ISOLATING" | "CONTAINED" | "CLEARED";
...
const isolatedTenants = new Set<string>();
/** Revoked key IDs — validated against on each request */
const revokedKeys     = new Set<string>();
/** Incident log */
const incidentLog: ContainmentEvent[] = [];

/** Rolling request-per-second counter per tenant */
const rpsCounters = new Map<string, { count: number; window_start: number }>();
...
export function evaluateForContainment(params: {
  ...
  latency_ms:  number;
  prompt_size: number;
}): ContainmentEvent | null {
  const { tenant_id, key_id, latency_ms, prompt_size } = params;

  // Skip if tenant already isolated
  if (isolatedTenants.has(tenant_id)) return null;

  let reason: TripwireReason | null = null;
  ...
  if (now - rps.window_start > 1000) {
    ...
    if (rps.count > 50) reason = "RAPID_ENUMERATION";
  }

  // Tripwire 2: Suspiciously large harvest (>50k chars in response)
  if (prompt_size > 50_000) reason = "ANOMALOUS_RESPONSE_HARVEST";

  // Tripwire 3: Near-zero latency (cache-poisoning / replay attempt)
  if (latency_ms < 5 && latency_ms >= 0) reason = "EXECUTION_STORM";
...
function fireIncidentAlert(event: ContainmentEvent): void {
  // In production: POST to PagerDuty / Opsgenie / Slack webhook
  console.error(
    `[C01:IncidentLogger] CRITICAL security event logged: ` +
...
export function isTenantIsolated(tenant_id: string): boolean {
  return isolatedTenants.has(tenant_id);
}

/** Returns true if this key has been revoked by containment */
export function isKeyRevoked(key_id: string): boolean {
...
export function clearIsolation(tenant_id: string): void {

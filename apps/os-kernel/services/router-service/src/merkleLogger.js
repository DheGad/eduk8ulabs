 ...
 */
export function computeLeafHash(receipt) {
...
export class MerkleTreeManager {
    ...
    treeKey(tenant_id, date) {
        return `${tenant_id}:${date}`;
    }
    getOrCreateTree(tenant_id, date) {
     ...
     */
    appendReceipt(tenant_id, receipt) {
     ...
     */
    getDailyRootHash(tenant_id, date) {
     ...
     */
    generateAuditProof(tenant_id, signature, date) {
        const d = date ?? MerkleTreeManager.todayUTC();
    ...
    importSnapshot(snapshot) {
        ...
        if (snapshot.root_hash !== null && tree.root_hash !== snapshot.root_hash) {
            console.error(`[MerkleLogger] INTEGRITY FAILURE: snapshot root=${snapshot.root_hash} ` +
...
if (isMain) {
    ...
    const tampered = receipts.map((r, i) => i === 1

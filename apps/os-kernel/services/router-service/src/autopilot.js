 ...
 */
function calculateOptimalRoute(schemaRecommendations, maxCostCents = 10.0, maxLatencyMs = 2000) {
    let bestScore = -1;
    let winner = MOCK_TELEMETRY[1]; // default to gpt-4o-mini
    for (const node of MOCK_TELEMETRY) {
        if (node.cost_per_1k > maxCostCents)
            continue;
        if (node.latency_ms > maxLatencyMs)
            continue;

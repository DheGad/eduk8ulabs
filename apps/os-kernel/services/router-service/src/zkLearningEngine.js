...
let baselineLatencyMs = null; // Used to compute efficiency %
// ─── HELPERS ─────────────────────────────────────────────────────────────────
/**
 ...
 */
function ewma(current, newSample) {
 ...
 */
function computeRoutingScore(w) {
 ...
 */
export function ingestTelemetry(metrics) {
    ...
    if (eventLog.length > MAX_EVENT_LOG)
        eventLog.shift(); // circular buffer
    ...
    console.info(`[V32:ZK-LearningEngine] Ingested hash=${execHash} ` +
 ...
 */
export function getModelWeight(model_id, provider) {
 ...
 */
export function getAllModelWeights() {
...
export function getNetworkStats() {
    ...
    if (baselineLatencyMs && baselineLatencyMs > 0 && weights.size > 0) {
        ...
        if (bestWeight) {
            efficiency = Math.max(0, ((baselineLatencyMs - bestWeight.avg_latency_ms) / baselineLatencyMs) * 100);
        }
    }
    return {

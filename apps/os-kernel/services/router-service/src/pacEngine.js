...
function evaluateCondition(condition, ctx) {
    ...
    switch (op) {
        ...
        case "neq":
            return normaliseStr(raw) !== normaliseStr(condition.value);
        case "in": {
            const list = condition.value.map(v => normaliseStr(v));
...
export function evaluatePolicy(ctx, policy) {
    ...
    const sorted = [...policy.rules].sort((a, b) => {
        ...
        if (a.action === "DENY" && b.action === "ALLOW")
            return -1;
        if (a.action === "ALLOW" && b.action === "DENY")
            return 1;
        return 0;
    });
 ...
 */
export const POLICY_FINANCE_STRICT = {
    version: "2.0.0",
    tenant_id: "ANY",
 ...
 */
export const POLICY_DEFENSE_ITAR = {
    version: "2.0.0",
    tenant_id: "ANY",
...
export const POLICY_GENERIC_BASELINE = {
    version: "1.0.0",
    tenant_id: "ANY",

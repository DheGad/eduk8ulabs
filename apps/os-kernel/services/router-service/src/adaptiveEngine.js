...
// ─── Helpers ──────────────────────────────────────────────────────────────────
function gaussianVariate(mean, stdDev) {
 ...
 */
export async function benchmarkNewModel(model_id, provider, display_name) {
    ...
    for (const probe of SYNTHETIC_JAILBREAK_PROBES) {
        ...
        if (refused)
            refusals++;
        console.info(`[V31:AdaptiveEngine]   → Probe "${probe.slice(0, 50)}…" ` +
    ...
    const compositeScore = Math.round(0.35 * latencyScore + 0.25 * costScore + 0.40 * riskScore);
    ...
    console.info(`[V31:AdaptiveEngine] ✅ Benchmark complete in ${totalBenchmarkMs}ms | ` +
...
function injectIntoRegistry(profile, costRank) {
    ...
    let tier = "CLOUD_CONSUMER";
    if (profile.risk_score >= 90)
        tier = "CLOUD_ENTERPRISE";
    else if (profile.risk_score >= 75)
        tier = "CLOUD_ENTERPRISE";
    ...
    console.info(`[V31:AdaptiveEngine] 💉 Dynamically injected: ` +

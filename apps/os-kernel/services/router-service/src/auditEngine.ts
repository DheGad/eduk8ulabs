...
export interface AuditReport {
  ...
  generated_at: string;
  metrics: {
    total_executions_audited: number;
    average_trust_integrity: number;       // From V25
    cryptographic_chain_consistency: "100% VERIFIED" | "FAILED"; // From V13/V14
    data_residency_compliance: string[];   // From V23
    zero_knowledge_leakage: "0 BYTES";     // From V32
  };
  signatures: {
 ...
 */
export async function generateAuditReport(tenant_id: string, timeframe: string): Promise<AuditReport> {

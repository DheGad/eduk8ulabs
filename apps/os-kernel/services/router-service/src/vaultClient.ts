...
// CONFIG
// ----------------------------------------------------------------
const VAULT_SERVICE_BASE_URL =
  process.env.VAULT_SERVICE_URL ?? "http://localhost:4002";
...
export async function fetchDecryptedKey(
  ...
  if (!internalSecret) {
    throw new VaultClientError(
      "VAULT_AUTH_FAILURE",
      "[RouterService:vaultClient] FATAL: INTERNAL_ROUTER_SECRET is not configured. " +
  ...
  try {
    const response = await axios.get<{ success: true; decrypted_key: string }>(
      url,
      {
        headers: {
    ...
    if (!decryptedKey || typeof decryptedKey !== "string") {
      throw new VaultClientError(
        "VAULT_ERROR",
        "Vault Service returned a malformed response — decrypted_key missing."
      );
    }
    ...
    if (!axiosErr.response) {
      ...
      throw new VaultClientError(
        "VAULT_UNREACHABLE",
        `Cannot reach Vault Service at ${VAULT_SERVICE_BASE_URL}. ` +
    ...
    if (statusCode === 404) {
      throw new VaultClientError(
        "KEY_NOT_FOUND",
        `No BYOK key found for provider "${provider}". ` +
    ...
    if (statusCode === 403) {
      ...
      console.error(
        `[RouterService:vaultClient] CRITICAL: Vault Service rejected our internal token ` +
          `(403). INTERNAL_ROUTER_SECRET mismatch between router-service and vault-service.`
      );
      throw new VaultClientError(
        "VAULT_AUTH_FAILURE",
        "Internal service authentication failed. Contact the platform administrator."
      );
    }

    if (statusCode === 500 && errorCode === "VAULT_INTEGRITY_FAILURE") {
      console.error(
        `[RouterService:vaultClient] CRITICAL: Vault integrity failure for ` +
          `userId=${userId}, provider=${provider}. Possible DB tampering.`
      );
      throw new VaultClientError(
        "VAULT_INTEGRITY_FAILURE",
        "A cryptographic integrity failure was detected in the vault. " +
    ...
    throw new VaultClientError(
      "VAULT_ERROR",
      `Vault Service returned unexpected error [${statusCode}]: ${errorMsg}`
    );
  }
}

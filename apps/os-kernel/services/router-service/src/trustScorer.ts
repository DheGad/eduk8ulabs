...
export interface TrustScoreContext {
  /** V15: Byzantine consensus result */
  consensus: {
    votes:            number;
    total_nodes:      number;
  ...
  /** V17: Cognitive Governor evaluation */
  cognitive: {
    /** The AI response was safe */
    isSafe:     boolean;
    /** 0–100 confidence that the response meets safety standards */
    confidence: number;
    reason:     string;
  } | null;

  /** V12: Policy-as-Code matched rule */
  policy: {
    /** The rule ID that allowed this request, e.g. "RULE_001" or "GENERIC" */
    matched_rule_id: string | null;
    /** "ALLOW" or "DENY" action taken */
    action: string;
  } | null;
}

// ----------------------------------------------------------------
// PENALTY BREAKDOWN (returned alongside the score for transparency)
// ----------------------------------------------------------------
export interface TrustScoreBreakdown {
  ...
  v12_penalty:      number;
  final_score:      number;
  /** Human-readable audit trail of why points were deducted */
  audit_notes:      string[];
}
 ...
 */
export function calculateGlobalTrustScore(context: TrustScoreContext): {
  score:     number;
  ...
  if (context.consensus) {
    ...
    if (votes <= quorum_required) {
      v15_penalty += 10;
      audit_notes.push(
        `V15: Weak quorum — ${votes}/${quorum_required} votes met (minimum threshold). −10pts`
      );
    }
    if (dissenting_count > 0 && v15_penalty === 0) {
      ...
      v15_penalty += 5;
      audit_notes.push(
        `V15: ${dissenting_count} dissenting node(s) detected despite strong quorum. −5pts`
      );
    }
  } else {
  ...
  if (context.cognitive) {
    ...
    if (v17_penalty > 0) {
      audit_notes.push(
        `V17: Cognitive confidence ${c}% → −${v17_penalty}pts (safety=${isSafe ? "PASS" : "FAIL"})`
      );
    }
  } else {
  ...
  if (context.policy) {
    ...
    if (GENERIC_RULE_IDS.has(ruleId)) {
      v12_penalty = 5;
      audit_notes.push(
        `V12: No targeted rule matched (rule_id="${context.policy.matched_rule_id ?? "null"}") — fallback policy applied. −5pts`
      );
    }
  } else {
 ...
 */
export function getTrustBand(score: number): "HIGH" | "MEDIUM" | "CRITICAL" {

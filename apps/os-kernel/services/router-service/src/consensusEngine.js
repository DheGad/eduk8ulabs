...
export class CognitiveConsensus {
     ...
     */
    async requestConsensus(userPayload) {
     ...
     */
    async mockTargetCall(nodeId, model, payload, forceAnomaly) {
    ...
    evaluateAgreement(responses) {
        ...
        responses.forEach(r => {
            ...
            if (hash === bestHash) {
                if (!targetPayload)
                    targetPayload = r.payload; // grab the first valid payload to pass down the pipe
            }
            else {
                outlier = r.nodeId;
            }

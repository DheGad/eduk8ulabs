...
// CONFIG
// ================================================================
const CACHE_ENABLED = process.env.CACHE_ENABLED !== "false"; // defaults true
const TTL_SECONDS = parseInt(process.env.CACHE_TTL_SECONDS ?? String(24 * 60 * 60), // 24 hours
10);
...
function getRedisClient() {
    ...
    try {
        ...
        console.warn("[SemanticCache] Failed to initialize Redis client:", err.message);
        return null;
    }
    return redisClient;
}
 ...
 */
export function buildCacheKey(provider, model, prompt) {
 ...
 */
export async function getCachedResponse(provider, model, prompt) {
    ...
    try {
        ...
        if (cached !== null) {
            console.log(`[SemanticCache] ✨ Cache HIT — key=${key.slice(0, 24)}... ` +
                `[${provider}/${model}] len=${cached.length}`);
            return cached;
        }
        console.log(`[SemanticCache] Cache MISS — key=${key.slice(0, 24)}... [${provider}/${model}]`);
        return null;
    }
    catch (err) {
        // Cache failure must NEVER break an LLM execution
        console.warn(`[SemanticCache] GET failed (non-fatal): ${err.message}`);
        return null;
    }
}
// ================================================================
 ...
 */
export async function setCachedResponse(provider, model, prompt, rawOutput) {
    const redis = getRedisClient();
    if (!redis)
        return;
    const key = buildCacheKey(provider, model, prompt);
    try {
        ...
        console.log(`[SemanticCache] 💾 Cached — key=${key.slice(0, 24)}... ` +
        ...
        console.warn(`[SemanticCache] SET failed (non-fatal): ${err.message}`);
    }
}
// ================================================================

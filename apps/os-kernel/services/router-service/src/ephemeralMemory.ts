...
interface EphemeralRegistration {
  exec_id:      string;
  manifest_ids: string[];    // Fragment manifests to destroy
  buffers:      Buffer[];    // Raw Node.js Buffers to overwrite
  registered_at: number;
  destroyed_at?: number;
}

/** In-memory ephemeral registry — keyed by execution certificate ID */
const registry = new Map<string, EphemeralRegistration>();
...
export function registerEphemeral(params: {
  ...
  if (existing) {
    if (params.manifest_ids) existing.manifest_ids.push(...params.manifest_ids);
    if (params.buffers) existing.buffers.push(...params.buffers);
  } else {
 ...
 */
export function zeroize(exec_id: string): { buffers_zeroized: number; manifests_destroyed: number } {
  ...
  for (const mid of reg.manifest_ids) {
    if (destroyManifest(mid)) manifestsDestroyed++;
  }
  ...
  console.info(
    `[V37:EphemeralMemory] Zeroized exec=${exec_id} | ` +
 ...
 */
export function purgeStaleRegistrations(maxAgeMs = 30_000): number {
  let purged = 0;
  const now  = Date.now();
...
export function getEphemeralStats(): {
  ...
  total_buffers_zeroized:   number;
  total_manifests_destroyed: number;
} {
  return {
